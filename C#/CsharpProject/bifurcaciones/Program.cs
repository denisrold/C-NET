/*
Especificaciones: 
desarrollar una característica que garantice que las edades de los animales y las descripciones físicas estén completas.
...
Informe al usuario de la aplicación cuando se cumplan todos los requisitos de datos y ponga la aplicación en pausa para asegurarse de que se puede ver y responder al mensaje.
                ...
                do
                {
                    ...
                    if (readResult != null)
                    {
                        ...
                        if (animalPhysicalDescription == "")
                        {
                            ...
                            string[] prueba = animalPhysicalDescription.Split(' ');
                            if(animalPhysicalDescription.Length >= 11){
        ...
        case "3":
            ...
            for (int i =0 ; i < maxPets ; i++){
                if(ourAnimals[i,2] == "Age= ?" && ourAnimals[i,0] != "ID #: "){
                    do{
                       Console.WriteLine($"Ënter a Age for {ourAnimals[i,0]}");
                       readResult = Console.ReadLine();
                       if (readResult !=null){
                            animalAge = readResult;
                            validEntry = TryParse(animalAge,out petAge);
                       }
                    }
                    while(validEntry ==false);
